...
/**
 * renderDiff(target, diffInput, opts)
 *   target: DOM element, string ID, or CSS selector
 *   diffInput: array of diff lines, OR a single string (will be split on \n)
 *   opts.keepImports: when true, import lines are not filtered out (keeps line
 *     numbering exact — required for anchored hunk views).
 */
function renderDiff(target, diffInput, opts) {
  opts = opts || {};
  var el;
  ...
  var filtered = opts.keepImports ? lines.slice() : lines.filter(function(l) {
  ...
  for (var ri = 0; ri < parsed.length; ri++) {
    ...
    if (p.type === 'hunk') {
      ...
      rows.push('<tr class="'+cls+'" data-new="'+p.newLine+'"><td class="diff-ln"></td><td class="diff-ln">'+p.newLine+'</td><td class="diff-code">'+esc(p.code)+'</td></tr>');
    } else if (p.type === 'del') {
      ...
      rows.push('<tr class="'+cls2+'" data-old="'+p.oldLine+'"><td class="diff-ln">'+p.oldLine+'</td><td class="diff-ln"></td><td class="diff-code">'+esc(p.code)+'</td></tr>');
    } else {
      rows.push('<tr class="diff-ctx" data-old="'+p.oldLine+'" data-new="'+p.newLine+'"><td class="diff-ln">'+p.oldLine+'</td><td class="diff-ln">'+p.newLine+'</td><td class="diff-code">'+esc(p.code)+'</td></tr>');
    }
  }
...
/**
 * splitHunks(patch) -> [{header, lines, newStart, newEnd, oldStart, oldEnd}]
 */
function splitHunks(patch) {
  var lines = toLines(patch), hunks = [], cur = null;
  for (var i = 0; i < lines.length; i++) {
    var l = lines[i];
    var m = l.match(/^@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/);
    if (m) {
      var oS = parseInt(m[1], 10), oN = m[2] === undefined ? 1 : parseInt(m[2], 10);
      var nS = parseInt(m[3], 10), nN = m[4] === undefined ? 1 : parseInt(m[4], 10);
      cur = { header: l, lines: [l], oldStart: oS, oldEnd: oS + Math.max(oN, 1) - 1, newStart: nS, newEnd: nS + Math.max(nN, 1) - 1 };
      hunks.push(cur);
    } else if (cur) {
...
/**
 * renderHunk(target, patch, startLine, endLine)
 *   Renders only the hunk(s) of `patch` whose new-file range intersects
 *   [startLine, endLine], with those rows tagged `diff-anchor`. When no hunk
 *   intersects (finding on an unchanged line), renders the nearest hunk and
 *   returns { nearest: true, distance }. Returns null when the patch is empty.
 */
function renderHunk(target, patch, startLine, endLine) {
  var el = (typeof target === 'string') ? (document.getElementById(target) || document.querySelector(target)) : target;
  if (!el) return null;
  var hunks = splitHunks(patch);
  if (!hunks.length) { el.innerHTML = '<div class="diff-empty">No diff data for this file (binary, too large, or not in the PR).</div>'; return null; }
  var s = Math.min(startLine, endLine), e = Math.max(startLine, endLine);
  var hit = hunks.filter(function(h) { return h.newEnd >= s && h.newStart <= e; });
  var info = { nearest: false, distance: 0 };
  if (!hit.length) {
    var best = null, bestD = Infinity;
    for (var i = 0; i < hunks.length; i++) {
      var h = hunks[i];
      var d = (s > h.newEnd) ? s - h.newEnd : (e < h.newStart ? h.newStart - e : 0);
      if (d < bestD) { bestD = d; best = h; }
    }
    hit = [best]; info.nearest = true; info.distance = bestD;
  }
  var out = [];
  hit.forEach(function(h) { out = out.concat(h.lines); });
  renderDiff(el, out, { keepImports: true });
  if (!info.nearest) {
    var rows = el.querySelectorAll('tr[data-new]');
    for (var r = 0; r < rows.length; r++) {
      var n = parseInt(rows[r].getAttribute('data-new'), 10);
      if (n >= s && n <= e) rows[r].classList.add('diff-anchor');
    }
  }
  return info;
}
