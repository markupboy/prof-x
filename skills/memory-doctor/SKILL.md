...
version: 1.0.0
description: |
  Audit the current project's agent-memory and, block by block, relocate each
  entry into a user-controlled home (project doc/skill/rule or user-level
  skill/rule) or archive it — draining memory so nothing uncontrolled
  accumulates in the agent's context. Use when asked to "drain memory", "audit
  agent memory", "memory doctor", or "/memory-doctor".
allowed-tools:
  - Read
  - Write
  - Edit
