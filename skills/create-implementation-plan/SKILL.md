...
version: 1.0.0
description: |
  Turn a refined requirements document into a structured implementation PLAN.md
  a fresh session can execute. Planning only — decides the "how", not the
  "what". Use when asked to "write the implementation plan", "plan from these
  requirements", "create a PLAN.md", or "/create-implementation-plan".
allowed-tools:
  - Read
  - Write
  - Grep
