...
Parse the argument to determine the time window. Default to 7 days if no argument given. Use `--since="N days ago"`, `--since="N hours ago"`, or `--since="N weeks ago"` (for `w` units) for git log queries. All times should be reported in **Mountain time** (use `TZ=America/Denver` when converting timestamps).
...
#    Use TZ=America/Denver for Mountain time conversion
TZ=America/Denver git log origin/main --since="<window>" --format="%at|%ai|%s" | sort -n
...
Show hourly histogram in Mountain time using bar chart:
...
```bash
# Get all unique commit dates (Mountain time) — no hard cutoff
TZ=America/Denver git log origin/main --format="%ad" --date=format:"%Y-%m-%d" | sort -u
```
...
- Convert all timestamps to Mountain time for display (use `TZ=America/Denver`)
